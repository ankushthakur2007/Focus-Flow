...
-- 2. Drop any views that depend on task_shares
...
-- 4. Drop the task_share_activities table if it exists
DROP TABLE IF EXISTS task_share_activities;
...
-- 9. Create new RLS policies for task_chats if the table exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_chats') THEN
    EXECUTE '
      ...
      CREATE POLICY "Users can insert chats for their own tasks"
