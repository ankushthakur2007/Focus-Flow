...
export default async function handler(
  ...
  try {
    ...
    // 4. Create a function to find users by email case-insensitively
    const { error: functionError } = await supabase.rpc('exec_sql', {
      sql_query: `
        ...
        SECURITY DEFINER
        AS $$
          SELECT * FROM profiles WHERE LOWER(email) = LOWER(search_email);
        $$;
    ...
    // 5. Check if there are any shared tasks with this user
    ...
    // 6. Get the actual tasks to ensure we have complete task data
    let tasks = [];
    ...
    res.status(200).json({ 
      message: 'Successfully fixed shared tasks and redirect',
